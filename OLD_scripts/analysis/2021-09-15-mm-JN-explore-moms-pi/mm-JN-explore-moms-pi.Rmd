...
## we shoud check which patient has the highest number of FC differs
sort(table(all_together_pruned_difs$host_subject_id))

## lets check out patient EP353148
View(all_together_pruned_difs[which(all_together_pruned_difs$host_subject_id=="EP353148"),])
...
## pruning to just the control taxxa
```{r}
x <- list(Calibrated = ps.cal, Observed = ps.obs) %>%
  #remove taxa that were not in the 2015 paper
  map(~prune_taxa(control_species, .)) %>%
...
ep353148 <-  x %>%
  filter(host_subject_id == "EP353148") %>%
  ggplot(aes(host_visit_number, .abundance, color = type)) +
  scale_y_log10() +
  facet_grid(otu_abbrev~., scales = 'fixed') +
  geom_line(aes(group = type))
...
ep353148 / m2_ep353 + plot_layout(heights=c(1,0.1))
```
...
#take in split FC list and report summary stats of FC diff
get_FC_diff <- function(x){
...
#lets split this df into seperate dfs based on the taxa being looked at
...
#get total number of samples available for each subject with more than 1 sample provided
x_filt <- x %>% filter(type=="Calibrated") %>% filter(host_subject_id %in% Subject_names)
length(unique(x_filt$host_subject_id))


#total number of unique samples - the number of subjects (i.e. number of samples that come from the 1st visit of that subject) (as FC change cannot be calualted for the 1st visit)
total_samps_minus_first <- length(unique(x_filt$.sample))- length(unique(x_filt$host_subject_id))

#percent of times there is an FC difference for each of the seven taxon that were included in the original mock.
#NOTE that this number would be higher in practice as we didn't remove cases were the abundance was 0 in the first and second sample 0 (i.e. no possible way to have FC dif 
# between calibrated and observed)
barplot(total_samples_per_taxa/total_samps_minus_first*100)

## look at magnitude of FC diff when there is a difference in FC direction for each taxa
ggplot(FC_dif_pruned_data, aes(x=.otu, y=as.numeric(FC_Diff))) + geom_boxplot() + theme_cowplot() + ylab("Fold Change Difference when FC direction is different") + 
