...
get_taxa_fold_change <- function(cal, obs, subject, prune=F, cntrl_spec=control_species){
  ...
  #okay for each of these taxa we now need to obsculate the fold change in abundance between row i and row i + 1
  for(i in taxa_names){
...
all_together <- do.call(rbind, pruned_sign_dif)
...
all_together <- do.call(rbind, nonpruned_sign_dif)
